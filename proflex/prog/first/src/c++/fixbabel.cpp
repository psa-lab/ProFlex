...
void list::make_connectivity(int usage) {
  ...
  float temp, dist[1000], **sxyz, no_bonds = 0, *chk_bond_dist, *poor_bond_dist,
                                  factor;
    ...
    if (flag == 1) {
      ...
      while (tempo->r1.mod_res_no == res_ptr->r1.mod_res_no &&
        ...
        if (tempo->r1.aname[2] != 'H' && tempo->r1.aname[2] != 'D') {
          // AJR 05.13.02 deal with labeling of known amino acid residue as O'
          // instead of O: relabel
          ...
          if (flag17 != 1) {
            for (i = 1; i <= res[rnumber].natom_sum; i++) {
              ...
              type[2] = '\0';
              if (strncmp(name, type, 2) == 0 && res[rnumber].occu[i] != 1) {
                A_ptr[i] = tempo;
                atom_found = 1; //---------known non-hydrogen atom
                res[rnumber].occu[i] = 1;
                natom++;
          ...
          if (atom_found != 1) { //-------------------extra atoms
            if (strcmp(tempo->r1.aname, "  OT1") == 0 ||
                ...
                strcmp(tempo->r1.aname, "  O''") ==
                    0) { // AJR 05.13.02 just rewrite as OXT
      ...
      //------------------End of group - Now work with known
      //group------------------//
            ...
            if (res[rnumber].occu[res[rnumber].A_link[i][j]] == 1) {
              ...
              if (i == 1) {
                if (strcmp(p[so]->r1.chain, p[sf]->r1.chain) == 0) {
                  ret_val = check_dist(so, sf, temp, atype[so], atype[sf], 1,
                                       p[so]->r1.rname, p[so]->r1.res_no,
                                       p[so]->r1.code, p[so]->r1.chain,
                                       p[sf]->r1.rname, p[sf]->r1.res_no,
                                       p[sf]->r1.code, p[sf]->r1.chain, usage);
                  if (ret_val == 1) {
                  ...
                  ret_val = check_dist(so, sf, temp, atype[so], atype[sf], 3,
                                       p[so]->r1.rname, p[so]->r1.res_no,
                                       p[so]->r1.code, p[so]->r1.chain,
                                       p[sf]->r1.rname, p[sf]->r1.res_no,
                                       p[sf]->r1.code, p[sf]->r1.chain, usage);
                ...
                if (ret_val == 1) {
                  ...
                  if (strcmp(res[rnumber].rname, "ARG") == 0) {
                    if (strcmp(atype[so], "  CZ ") == 0) {
                      if (strcmp(atype[sf], "  NH1") == 0 ||
                          strcmp(atype[sf], "  NH2") == 0 ||
      ...
      for (i = 1; i <= nhydro; i++) {
        ...
        for (j = 1; j <= res[rnumber].natom_sum; j++) {
          if (res[rnumber].occu[j] == 1) {
            ...
            // 2008:10 SN	Add a diagnostic message to the user if bond length
            // < 1.0
            if (temp >= 0.85 && temp < 0.995 &&
                strncmp(A_ptr[j]->r1.aname, "  C", 3) == 0) {
              cout << "\nWARNING: Distance between " << so << " and " << sf
                   << " is";
            ...
            ret_val =
                check_dist(so, sf, temp, atype[so], atype[sf], 2,
                           p[so]->r1.rname, p[so]->r1.res_no, p[so]->r1.code,
                           p[so]->r1.chain, p[sf]->r1.rname, p[sf]->r1.res_no,
                           p[sf]->r1.code, p[sf]->r1.chain, usage);
          ...
          while (sf != 0) {
            if (atype[sf][2] != 'H' && atype[sf][2] != 'D') {
              ...
              if (flag12 != 1) {
                ...
                temp = sqrt(temp);
                ret_val = check_dist(so, sf, temp, atype[so], atype[sf], 0,
                                     p[so]->r1.rname, p[so]->r1.res_no,
                                     p[so]->r1.code, p[so]->r1.chain,
                                     p[sf]->r1.rname, p[sf]->r1.res_no,
                                     p[sf]->r1.code, p[sf]->r1.chain, usage);
        ...
        for (jz = -1; jz <= 1; jz++) {
          ...
          while (sf != 0) {
            if (atype[sf][2] != 'H' && atype[sf][2] != 'D') {
              ...
              temp = sqrt(temp);
              ret_val =
                  check_dist(so, sf, temp, atype[so], atype[sf], 0,
                             p[so]->r1.rname, p[so]->r1.res_no, p[so]->r1.code,
                             p[so]->r1.chain, p[sf]->r1.rname, p[sf]->r1.res_no,
                             p[sf]->r1.code, p[sf]->r1.chain, usage);
    ...
    if (count > 1) {
      {
        if (flag7 == 0) {
          ...
          cout << endl
               << "Warning: The following hydrogen atoms can be"
  ...
  if (usage == 0) {
    ...
    for (i = 1; i <= no_atoms; i++) {
        ...
        if (mult[i] > 1) {
          ...
          for (j = 1; j <= mult[i]; j++) {
            if (strcmp(p[i]->r1.res_no, p[temp_link[i][j]]->r1.res_no) != 0) {
              if (flag9 == 0) {
                cout << endl
                     << "Possible incorrect bond(s) with atom(s):" << endl;
                flag9 = 1;
              }
              sf = temp_link[i][j];
              cout << "\t Atom#  Atom  Res    Res#    Chain" << endl;
              write_output(1, sf, 0, atype[sf], p[sf]->r1.rname,
                           p[sf]->r1.res_no, p[sf]->r1.code, p[sf]->r1.chain,
                           atype[sf], atype[sf], atype[sf], atype[sf],
                           atype[sf]);
              ans = '@';
      ...
      //	--- 2007:06 SN	---	Discrepancy in valency checks between v4
      //and v5
      else if (atype[i][2] == 'O') {
        if (mult[i] > 2) {
          flag10 = -1;
    ...
    //-------------copy the temp. connectivety table to new memory optimized
    //table.
    //------------------------------allocating memory for the new connectivity
    //table
    ...
    if (u_nrot_bond_count > 0) {
      for (i = 1; i <= u_nrot_bond_count; i++) {
        if (u_nrot_bond[i][1] != 0) {
          // changed 11.02.01 AJR to only prompt for this double bond lock if a
          // peptide bond.
          if ((atype[u_nrot_bond[i][1]][2] == 'C' &&
               atype[u_nrot_bond[i][2]][2] != 'N') ||
              (atype[u_nrot_bond[i][1]][2] == 'N' &&
               atype[u_nrot_bond[i][2]][2] != 'C')) {
            so = u_nrot_bond[i][1];
            sf = u_nrot_bond[i][2];
            ...
            if (flag9 == 0) {
              ...
              cout << endl
                   << "Should the following bond be considered as "
                   << "Non-Rotating?" << endl;
              flag9 = 1;
            }

            cout << endl << "\t Atom#  Atom  Res    Res#    Chain" << endl;
            write_output(1, so, sf, atype[so], p[so]->r1.rname,
                         p[so]->r1.res_no, p[so]->r1.code, p[so]->r1.chain,
                         atype[sf], p[sf]->r1.rname, p[sf]->r1.res_no,
                         p[sf]->r1.code, p[sf]->r1.chain);
            cout << "\tType y to LOCK the dihedral angle or any key to keep "
                    "unlocked: ";
  ...
  //---count no. of isolated water atoms, isolated Hydrogens and all isolated
  //atoms
  for (i = 1; i <= no_atoms; i++) {
    if (mult[i] == 0) {
      all_iso++;
      if ((strcmp(p[i]->r1.rname, "HOH") == 0 ||
           strcmp(p[i]->r1.rname, "DOD") == 0) &&
          p[i]->r1.aname[2] == 'O') {
        O_iso++;
      }
    }
  ...
  for (i = 0; i < chaincount; i++) {
    tempo = chainptr[i];
    while (strcmp(tempo->r1.aname, "  N  ") !=
           0) { // catches end of ATOM records
      if (tempo == start) {
        flag8 = 1;
        break;
      }
      ...
      if (i != 0) {
        ...
        for (j = 0; j < 20; j++) {
          if (strcmp(tempo->r1.rname, res[j].rname) == 0) {
            ...
            tempo = tempo->prior;
            while (strcmp(tempo->r1.aname, "  O  ") != 0)
              tempo = tempo->prior;
            if (oxtflag == 0) {
              if (usage == 0) {
                cout << endl
                     << "\tThe final residue, # " << tempo->r1.res_no
                     << " of chain ID " << tempo->r1.chain << endl
                     << "\twas not identified as a terminal oxygen "
                     << "(OXT)." << endl
                     << "\t"
                     << "Enter (y)es if the main chain oxygen in this residue"
                     << endl
                     << "\tshould be regarded as a charged acceptor at the "
                        "C-terminus: "
                     << endl;
...
tempo->r1.DAH_type='E';
  ...
  for (i = 1; i <= no_atoms; i++) {
    for (j = 1; j <= mult[i]; j++) {
      if (link_noHB[i][j] > i) {
        yofil << "REMARK:CF " << setw(5) << i << " " << setw(5)
              << link_noHB[i][j] << endl;
      }
    }
  }
  ...
  write_HPCFbond();
  yofil << "REMARK:L:" << dash1 << dash2 << endl;
  //-------------------------------Append TF connectivity to the _proflexdataset
  //file
  flag9 = 0;
  ...
  //----------------------------------Writing CHECK_BONDING into _proflexdataset
  //file
  if (chk_bond_count > 0) {
    yofil
        << "REMARK:w:check_bonding:  Dist.  A1#   A1  Res1 Res1# C1 A2#   A2  "
        << "Res2 Res2# C2" << endl;
    for (i = 1; i <= chk_bond_count; i++) {
      if (chk_bond[i][1] != -1) {
        ...
        yofil << "REMARK:W:check_bonding:"
              << setiosflags(ios::showpoint | ios::fixed) << setprecision(5)
              << setw(5) << chk_bond_dist[i] << setw(5) << so << atype[so]
              << "  " << p[so]->r1.rname << " " << p[so]->r1.res_no
  ...
  //--------------------------------Write number of isolated water in
  //_proflexdataset
  if (all_iso > 0) {
    yofil << "REMARK:w:isolated_atoms: Number of isolated atoms = " << all_iso
          << endl;
    if (O_iso > 0) {
    ...
    if (skip_hydro_count > 0) {
      yofil
          << "REMARK:w:isolated_hydrogen: Number of isolated hydrogen atoms = "
          << skip_hydro_count << endl;
    }
    yofil << "REMARK:L:" << dash1 << dash2 << endl;
  }

  //------------------------------Write number of disulfide bonds in
  //_proflexdataset
  if (ns_bond > 0) {
  ...
  //--------------------------------Write SKIP_NONHYDROGEN atoms into
  //_proflexdataset
  if (skip_nhydro_count > 0) {
    ...
    for (i = 1; i <= skip_nhydro_count; i++) {
      ...
      yofil << "REMARK:W:skip_nonhydrogen:" << setw(5) << so << " " << atype[so]
            << "  " << p[so]->r1.rname << "  " << p[so]->r1.res_no
            << p[so]->r1.code[0] << p[so]->r1.chain << endl;
    }
    yofil << "REMARK:L:" << dash1 << dash2 << endl;
  }
  ...
  if (poor_bond_count > 0) {
    ...
    for (i = 1; i <= poor_bond_count; i++) {
      if (poor_bond[i][1] != -1) {
        so = poor_bond[i][1];
        sf = poor_bond[i][2];
        yofil << "REMARK:W:poor_bond:"
              << setiosflags(ios::showpoint | ios::fixed) << setprecision(5)
              << setw(10) << poor_bond_dist[i] << setw(5) << so << atype[so]
              << "  " << p[so]->r1.rname << "  " << p[so]->r1.res_no
              << p[so]->r1.code[0] << " " << p[so]->r1.chain;
        yofil << setw(5) << sf << atype[sf] << "  " << p[sf]->r1.rname << "  "
              << p[sf]->r1.res_no << p[sf]->r1.code[0] << " " << p[sf]->r1.chain
  ...
  if (ma_count > 0) {
    ...
    for (i = 1; i <= ma_count; i++) {
      so = missing[i];
      yofil << "REMARK:W:missing_atoms:"
            << "   " << missing_atoms[i] << "  " << p[so]->r1.rname << " "
            << p[so]->r1.res_no << p[so]->r1.code << "  " << p[so]->r1.chain
            << endl;
    }
    yofil << "REMARK:L:" << dash1 << dash2 << endl;
  ...
  if (altloccount > 0) {
    if (conf_option == 1) {
      ...
      for (i = 0; i < altloccount; i++) {
        yofil << "REMARK:W:conformation_auto:"
      ...
      yofil << "REMARK:w:conformation_user: Residue Residue no. Chain "
            << "Conformation" << endl;
      for (i = 0; i < altloccount; i++) {
        yofil << "REMARK:W:conformation_user:"
              << "   " << altlocptr[i]->r1.rname << "    "
              << altlocptr[i]->r1.res_no << altlocptr[i]->r1.code[0]
              << "        " << altlocptr[i]->r1.chain << "       "
              << conf_chosen[i] << endl;
  ...
  if (type == 1) {
    if (d < distance[a1][a2].d[0]) { //------dist<d_min
      if (usage) {
        ...
        cout << endl
             << "Caution: The distance of [d ="
             << setiosflags(ios::showpoint | ios::fixed) << setprecision(2)
             << setw(5) << d << " Ang.] seems low!" << endl;
        cout << "\t Atom#  Atom  Res    Res#    Chain" << endl;
        write_output(1, so, sf, atom1, rname1, res_no1, code1, chain1, atom2,
                     rname2, res_no2, code2, chain2);
      ...
      if (d > distance[a1][a2].d[3]) { //----------dist>d_max
        if (d < maxdist) {
          if (usage) {
            ...
            cout << endl
                 << "Caution: The distance of [d ="
                 << setiosflags(ios::showpoint | ios::fixed) << setprecision(3)
                 << setw(5) << d << " Ang.] seems high!" << endl;
            cout << "\t Atom#  Atom  Res    Res#    Chain" << endl;
            write_output(1, so, sf, atom1, rname1, res_no1, code1, chain1,
                         atom2, rname2, res_no2, code2, chain2);
            cout << "Type  n  NOT to connect: ";
            cin >> answer;
            if (strlen(answer) == 1)
              ans = answer[0];
    ...
    if (d < distance[a1][a2].d[0] ||
        d > distance[a1][a2].d[3]) { //------(dist < d_min) or (dist > d_max)
      return (0);                    //--------------don't make connection
    } else {
      return (1); //--------------make connection
    }
...
void list::make_connection(int *my_mult, int **temp_link, int so, int sf) {
  if (my_mult[so] >= maxr - 1) {
    ...
    cout << "Valency of atom # " << so << " is (" << my_mult[so]
         << ") greater than ";
    cout << maxr - 1 << "; therefore EXIT!" << endl;
    exit(20);
  }
  if (my_mult[sf] >= maxr - 1) {
    cout << "Valency of atom # " << sf << " is (" << my_mult[sf]
         << ")greater than ";
    cout << maxr - 1 << "; therefore EXIT!" << endl;
    exit(20);
  }
...
void list::write_output(int type, int so, int sf, char *atom1, char *rname1,
  ...
  if (type == 1) { //---------write to screen(one below the other)
    cout << "\t" << setw(5) << so << "  " << atom1 << "  " << rname1 << "    "
         << res_no1 << code1 << "  " << cid1 << endl;
    if (sf != 0) {
      cout << "\t" << setw(5) << sf << "  " << atom2 << "  " << rname2 << "    "
           << res_no2 << code2 << "  " << cid2 << endl;
    }
  }
