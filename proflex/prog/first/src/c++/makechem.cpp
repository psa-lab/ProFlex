...
void record::getrecord(void) {
  ...
  int i, l, is_atom = 0, is_hetatm = 0, is_ter = 0;
   ...
   * gets the model number, it sets the model no and returns  - Sameer Mar 4
   * 2004
   */
  ...
  while (!is_atom && !is_hetatm && !is_ter && !ifil.eof()) {
    ...
    if (strncmp(line, "CONECT", 6) && strncmp(line, "ANISOU", 6) &&
        strncmp(line, "MASTER", 6) && strncmp(line, "END", 3)) {
    ...
    // This will occur if the input buffer exceeds the buffer size of the
    // variable line. BMH 5.24.02.
    if (ifil.fail()) {
...
void list::add_record(int usage) {
  ...
  int flag2 = 1, file_open_flag = 0, flag1 = 0, i = 0, length = 0, k = 0, l = 0,
      m = 0, flag4 = 0;
...
// lend support to this hypothesis.
  ...
  ************************************************************/
  ...
  if (!start && !last) {
    cout << endl
         << "\a\tThere were no atom records in the PDB file!" << endl
         << endl;
    answer[0] = 'r';
    answer[1] = 'm';
    answer[2] = ' ';
  ...
  for (i = 0; i < chaincount; i++) {
    ...
    if (chainletter[i] == 'U' || chainletter[i] == 'V' ||
        chainletter[i] == 'W' || chainletter[i] == 'X' ||
        chainletter[i] == 'Y' || chainletter[i] == 'Z') {
  ...
  if (altloccount != 0) {
    ...
    //---------------------------- AJR 04.29.02 adding default, "-non", option
    //<-> usage = 1.
    cout << endl
         << "Multiple conformations were found for certain residues." << endl;
  ...
  /* _____ SN 2007:02 --- Check if the input file is missing even polar 'H'
     atoms Note that all ATOM recs are stored in chainptr[0] with consecutive
     records linked by "prior" pointer and not "next"!
  ...
  /* _____ SN 2008:02 --- Check all the chains!
                          ...
                          chain and usually are assigned non-standard rsd
     names!*/
  ...
  while (temp != NULL) {
    ...
    // _____ SN 2007:02 --- fix to catch water molecules with no 'H's in input
    // PDB file
    ...
    if (!strcmp(temp->r1.rname, "HOH") && temp->r1.aname[2] == 'O') {
      ...
      if ((temp->prior)->r1.aname[2] != 'H') {
        ...
        cout << "\n#                                                           "
                "   #";
        cout << "\n# WARNING: Input file is missing hydrogens on water "
                "molecules  #";
        cout << "\n#          Please add and equilibrate hydrogens using "
                "software #";
        cout << "\n#          such as AMBER, CHARMM, Reduce or WhatIf,         "
                "   #";
        cout << "\n#          delete the proflexdataset file, and re-run "
                "ProFlex  #";
        cout << "\n#                                                           "
                "   #";
        cout << "\n#          Make sure the ligand and other co-factor atoms "
                "are  #";
        cout << "\n#          appropriately protonated.                        "
                "   #";
        cout << "\n#                                                           "
        ...
        exit(1);
                          ...
                          // Assuming the rest of the waters have 'H's we break
                          // out of check
      break;
...
void list::del_conf(std::string choice_in, int usage) {
  ...
  case 1: //---------------------------------Default
  {
    ...
    for (i = 0; i < altloccount; i++) {
      ...
      // occu[0]=max_occ;
      for (j = 1; j < no_conf[i];
           j++) // Determine the conformation that has max. occupancy
      {
        temp = temp->prior;
        /* TESTING
           cout << "altloc = " << i << " conf. = "
        ...
        if (max_occ < temp->r1.occ) {
          ...
          t = temp;
          temp = temp->prior;
          // TESTING                      cout << "temp pointer changed!!" <<
          // endl;
      ...
      if (occ_flag == 0) // Determine the conformation that has highest mobility
      {
        ...
        /* TESTING
           for(j=0;j<no_conf[i];j++)
           {
           cout<<endl<<"Residue "<<i<<"conformation "<<j<<": sum of temp.=
           "<<temp_sum[j]<< endl;
           }
           END OF TESTING */
    ...
    if (prev_sel_flag == 0) {
      ...
      if (ans != 'n' && ans != 'N') {
        for (i = 0; i < altloccount; i++) {
          ...
          prev = temp->prior;
          while (temp->r1.mod_res_no == altlocptr[i]->r1.mod_res_no) {
            if (temp->r1.altloc[0] != prev_sel[i] &&
                strcmp(temp->r1.altloc, " ") != 0) {
              if (temp == altlocptr[i]) {
  ...
  case 2: //----------------------------------------------New selection
  {
    ...
    for (i = 0; i < altloccount; i++) {
      ...
      while (f == 0) {
        cout << endl
             << "Choose the conformation for residue "
             << altlocptr[i]->r1.res_no << altlocptr[i]->r1.code << " ("
    ...
    for (i = 0; i < altloccount; i++) {
      /*
        ...
        while(temp->r1.mod_res_no == altlocptr[i]->r1.mod_res_no)
        {
        cout << endl << "temp = " << temp << " prior = " << temp->prior << "
        next = " << temp->next << endl; cout << "temp:  " << "Res = " <<
        temp->r1.rname << " Res # = " << temp->r1.res_no << " conf = " <<
        temp->r1.altloc << " Atom = " << temp->r1.aname << " Atom # = " <<
        temp->r1.sr_no << endl; prev = temp->prior; cout << "prior: " << "Res =
        " << prev->r1.rname << " Res # = " << prev->r1.res_no << " conf = " <<
        prev->r1.altloc << " Atom = " << prev->r1.aname << " Atom # = " <<
        prev->r1.sr_no << endl; prev = temp->next; cout << "next:  " << "Res = "
        << prev->r1.rname << " Res # = " << prev->r1.res_no << " conf = " <<
        prev->r1.altloc << " Atom = " << prev->r1.aname << " Atom # = " <<
        prev->r1.sr_no << endl; prev=temp->prior; temp=prev;
        }
        cout << endl << endl;
        cin >> k;
        */
...
void list::make_chem_rec(void) {
  ...
  while (flag3 != 1) {
    if (tem == start) {
      ...
      xofil << tem->r1.field1 << setw(5) << tem->r1.ri_sr_no << tem->r1.aname
            << tem->r1.altloc << tem->r1.rname << tem->r1.chain
            << tem->r1.res_no << tem->r1.code << tem->r1.strx << tem->r1.stry
            << tem->r1.strz << tem->r1.occupancy << tem->r1.temp_string
            << setw(5) << tem->r1.mod_res_no << " " << tem->r1.DAH_type << endl;
      ...
      if (strncmp(tem->r1.field1, "TER", 3)) { // AJR 05.13.02 omit TER lines
        xofil << tem->r1.field1 << setw(5) << tem->r1.ri_sr_no << tem->r1.aname
              ...
              << tem->r1.res_no << tem->r1.code << tem->r1.strx << tem->r1.stry
              << tem->r1.strz << tem->r1.occupancy << tem->r1.temp_string
              << setw(5) << tem->r1.mod_res_no << " " << tem->r1.DAH_type
              << endl;
      }
      tem = tem->prior;
    }
...
void list::cal_maxmin(void) {
  ...
  while (flag3 != 1) {
    ...
    if (tem == start) {
      ...
      if (tem->r1.field1[0] !=
          'T') { // if the current list node is not a TER record in the pdb file
        no_atoms++;
        // cout << tem->r1.sr_no << endl;
      }
      if (tem->r1.coord[1] < xmin)
