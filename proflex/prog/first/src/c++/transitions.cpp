...
void list::read_proflexdataset(void) {
  ...
  //------since read_proflexdataset has been invoked, we know user is using
  // previous dataset - Sameer 12.Mar.04
  real_hb_no = 0;
  ...
  while (!fptr.eof()) {
    ...
    if (!strncmp(line, "ATOM", 4) || !strncmp(line, "HETATM", 6) ||
      ...
      strncpy(fresh->r1.field1, line,
              6); // PDB field identifier (ie. ATOM, HETATM, REMARK)
      ...
      strncpy(fresh->r1.occupancy, line + 54,
              6); // Temperature factor (B_value)
    ...
    // Read Hydrogen Bond constraints from proflexdataset file
      ...
      /* of these HB records, count only the hydrogen bonds and salt bridges
       * i.e. every HB record except the hydrophobic tethers - Sameer 12.Mar.04
       */
      ...
      if (id_hb < 1) {
        ...
        cout << "\t       FIRST must terminate until ID label is changed."
             << endl;
        cout << "\t       (Set ID > 0)" << endl << endl;
        exit(25);
      }
      if (id_hb >= no_atoms) {
        cout << endl
        ...
        cout << "\t       FIRST must terminate until ID label is changed."
             << endl;
        cout << "\t       (Set ID < " << no_atoms << ")" << endl << endl;
        exit(26);
      }
  ...
  //--------------------------------------------Check if any unknown H-bonds
  // detected
  if (unknown > 0) {
    cout << endl
         << "\a\tNumber of unknown hydrogen bond types detected = " << unknown
         << endl;
    cout << "\tGeometrical screening will proceed as if a salt bridge." << endl;
  ...
  for (i = 1; i <= no_atoms; i++) {
    ...
    for (j = 1; j <= mult[i]; j++) {
      ...
      if (iflag < 0) {
        ...
        if (so < sf) {
          cout << endl
               << "\tDuplicate covalent bond connection ignored!" << endl;
          cout << "\t Atom#  Atom  Res    Res#    Chain" << endl;
  ...
  for (i = 1; i <= nhb; i++) {
    ...
    if (temp_hb_id[hb_id[i]] > 1) {
      cout << endl
           << endl
           << "\a\tERROR: Degeneracy found in hydgrogen bond ID = " << hb_id[i]
           << " " << i << " " << temp_hb_id[hb_id[i]] << endl
           << "\t       FIRST must terminate until ID labeling is corrected!"
           << endl
           << endl;
      exit(27);
    }
...
void list::check(void) {
  ...
  int link_counter =
      0; // To count links before flagging valency warning - Sameer
  float tmp;
  //--------------------------------------------No groups in PDB file
  if (no_atoms == 0) {
    cout << endl
         << "STOP: Please check original PDB file!  No atoms found!" << endl;
    exit(11);
  }
  for (i = 1; i <= no_atoms; i++) {
  ...
  for (i = 1; i <= no_atoms; i++) {
    /*--------------------------- Real Valency check  ------------------*/
    /* The valency is taken as the no of bonds of an atom. This is fine, as long
     * as we don't add up the bonds to pseudo atoms as well. This becoms a
     * problem when re-using proflexdataset in a run as then bonds to psuedo
     * atoms too leads FIRST to decide valency as incorrectly high, and give
     * these warnings - Sameer
     */
    ...
    if (p[i]->r1.aname[2] == 'H' || p[i]->r1.aname[2] == 'D') {
      ...
      // if(mult[i]>1)
      {
        ...
        cout << "\t Atom#  Atom  Res    Res#    Chain" << endl;
        write_output(1, so, 0, p[so]->r1.aname, p[so]->r1.rname,
                     p[so]->r1.res_no, p[so]->r1.code, p[so]->r1.chain,
                     p[so]->r1.aname, p[so]->r1.aname, p[so]->r1.aname,
                     p[so]->r1.aname, p[so]->r1.aname);
        for (j = 1; j <= mult[i]; j++) {
          sf = link_noHB[i][j];
          write_output(1, sf, 0, p[sf]->r1.aname, p[sf]->r1.rname,
                       p[sf]->r1.res_no, p[sf]->r1.code, p[sf]->r1.chain,
  ...
  //------------------------------------Sort H-bonds from highest to lowest
  // energy
  //                                                    Use a Shell sort
  //                                                    algorithm
  hb_energy[0] = 1e25;
  ...
  if (nhb < (no_atoms) / 65) {
    cout << endl
         << endl
         << "WARNING:  Low number of hydrogen bonds found!  H-bond # = " << nhb
         << endl;
    if (nhb == 0) {
      exit(55);
    }
