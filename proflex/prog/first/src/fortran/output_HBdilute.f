...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      subroutine output_HBdilute(archive)
c ------------------------------------------------------------------------------
...
c ==============================================================================
      ...
      integer      torsion(2,nbmax),HBanalysis
      ...
      character*80 bondwt_file, decomp_file
...
c ------------------------------------------------------------ format statements
 ...
 8000 format(a15)
      ...
      open(9,file=bondwt_file, access='append')
c ---------------------------------------------------- temporarily change clst()
      ...
      if( k .ne. nclst ) then
         open(2,file='clst.err',status='new',access='append')
         write(2,*) k,nclst
         close(2)
         stop
      endif
...
cc ---------------------------------------------------------- GLOBAL ERROR CHECK
      ...
      enddo
c      write(54,*) nflop,kflop,natom,nhinge
c --------------------------------------------------------------- ERROR FOUND!!!
...
cc     use pointBrc as a chain for linking BULK sites in a cluster
cc     use pointArc to bin all BULK sites in a cluster
cc                                                                 define cname()
c      call colorclustr(clst,pointbrc,natom,pointarc,color,nclst,
c     &     mult,pointr,cname)
c
cc ------------------------------------- relabel sites for quick graphics display
cc
cc     INPUT:  use pointBrc as a chain for linking BULK sites in a cluster
cc     INPUT:  use pointArc to bin all BULK sites in a cluster
cc     NOTE:   pointArc,pointBrc are set up from subroutine colorclustr()
cc     OUTPUT: pointArc,pointBrc,oldlabel,newlabel,paint,ncolor,calow,cabig 
cc             and cslow,csbig
...
c      call decomp_list(oldlabel, natom, clst, nclst)
      call decomp_list(size, natom, clst, nclst)
