...
/* back to hbdilute.c                                                  */
/***********************************************************************/
void read_chem_file(atom_list **chem_file_head, FILE *pdb_file,
                    int y_translate[10], int chain_size[10], char chain_IDs[10],
                    int *number_of_chains, int *total_insertions,
                    ...
                    int missing_rsd_count[10], int *total_hbond_count) {
  ...
  /* SN 2007:12:19
     ...
     - Compare consecutive rsd numbers to track any
       missing rsds
   */
  int a = 0, is_atom = 0, known_insertion = 0;

  char linebuf[150], atom[4], chain_label;

  atom_list *temp = NULL, *current = NULL;
  ...
  *total_hbond_count = 0; /* SN 2008:05 for hbdilute plot */
  ...
  while (fgets(linebuf, sizeof(linebuf), pdb_file) != NULL) {
    ...
    if (is_atom &&
        (!strcmp(atom, "N") || !strcmp(atom, "CA") || !strcmp(atom, "C"))) {
      ...
      /*printf("residue: %d   total %d: number_of_insertions at residue %d:
         %3d\n", temp->residue, *total_insertions,
         insertion_res_num[(*total_insertions)-1],
            ...
            for (prev_rsd += 1; prev_rsd < current->residue; prev_rsd++, i++) {
              if (i == MISSING_RSD_LIST_SIZE) {
                ...
                printf("\n* WARNING: Missing residue list is full!             "
                       "        *");
                printf("\n*	       To prevent memory leak the program will "
                       "exit now! *");
                printf("\n*	       Please change MISSING_RSD_LIST_SIZE and "
                       "re-run.   *");
                printf("\n*****************************************************"
                       "*********\n");
                exit(0);
  ...
  for (a = 0; a < 100; a++) {
    if (number_of_insertions[a] > 0) {
      ...
      printf("\n   %d		  %c		  %d", insertion_res_num[a],
             insertion_chain_ID[a], number_of_insertions[a]);
#endif
  ...
  for (a = 0; a <= *number_of_chains; a++) {
...
#ifdef DEBUG_SN
    printf("\n  %d\t %c\t%d\t  %d\t %d\t %d\t %d\t%d", a, chain_IDs[a],
           insertion_count[a], chain_size[a], min_max_rsd[a][0],
           min_max_rsd[a][1],
    ...
    if (missing_rsd_list[i] > 0) {
      ...
      for (i = j; i < j + missing_rsd_count[a] && i < MISSING_RSD_LIST_SIZE;
           i++) {
        if (missing_rsd_list[i] > 0)
...
/**********************************************************************/
int compute_number_of_clusters(int atom_label[MAXATOMS], atom_list *chem_file,
                               int cluster_label[MAX_CLUSTER_COUNT],
                               int number_of_atoms) {

  int a = 0, tally = 0, current_number = 0, total_clusters = 0,
      current_label = 1, renew_list = 0, check = 0, minimum_cluster_size = 3,
...
void first_decomp(int long_output, int total_clusters,
                  int bulk_atom_label[MAXATOMS], atom_list *chem_file,
  ...
  clusters *nc_temp = NULL, *nc_current = NULL;

  start_of_list = chem_file;

  if (!long_output) {

    for (current_cluster = 0; current_cluster < total_clusters;
         current_cluster++) {
...
/**********************************************************************/
void set_new_decomp_info(int total_clusters, int bulk_atom_label[MAXATOMS],
                         int atom_num, clusters *new_decomp[MAX_CLUSTER_COUNT],
                         atom_list *chem_file,
                         int cluster_labels[MAX_CLUSTER_COUNT]) {

  int cluster_number = 0, current_cluster = 0, found_cluster = 0,
      this_cluster = 0;

  atom_list *start_of_list = NULL;

  clusters *nc_current = NULL, *nc_temp = NULL;
...
/**************************************************/
void swap_cluster_info(clusters *array[MAX_CLUSTER_COUNT], int element_A,
                       int element_B, int size[MAX_CLUSTER_COUNT]) {
...
/**************************************************/
void bsort(clusters *array[MAX_CLUSTER_COUNT], int size[MAX_CLUSTER_COUNT],
           int start, int end) {
...
void compute_new_cluster_list(clusters *nc_list[MAX_CLUSTER_COUNT], int num_new,
  ...
  int a = 0, b = 0, c = 0, d = 0, e = 0, total_count = 0, color_count = 1,
      is_a_subset = 0, subset_index = 0, subset_element = 0,
      total_atoms[MAX_CLUSTER_COUNT], current_color_index = 0, atom_count = 0,
      sort_color[MAX_CLUSTER_COUNT], t = 0, new_colors[MAX_CLUSTER_COUNT];

  clusters *new_current, *old_current, *subset_list[MAX_CLUSTER_COUNT],
...
/**********************************************************************/
int compare_old_and_new_decomps(clusters *new[MAX_CLUSTER_COUNT],
                                clusters *old[MAX_CLUSTER_COUNT],
                                int cluster_counter,
                                int cluster_sizes[MAX_CLUSTER_COUNT],
                                int old_cluster_count) {

  int a = 0, size_new = 0, size_old = 0, this_cluster_size = 0;

  clusters *nc_current = NULL, *oc_current = NULL;
  ...
  */
  ...
  for (a = 0; a < cluster_counter; a++) {
    ...
    while (nc_current) {
       ...
       * nc_current->atom_number, nc_current->residue_number,
       * nc_current->chain_ID );*/
      fflush(stdout);
      this_cluster_size++;
      nc_current = nc_current->next_element;
  ...
  /*
   ...
   cluster_counter, size_old, old_cluster_count );*/
...
/**********************************************************************/
void set_temp_files(FILE *file_list[FILE_LIST_SIZE], int *number_of_pages,
                    int *ps_page_number) {
