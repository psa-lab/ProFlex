...
void print_text_decomp(clusters *nc_count[120], int cluster_counter,
                       int line_number, FILE *text_file, int y_translate[10],
                       int number_of_residues, int hb_number, float hb_energy,
                       float mean_coordination) {

  int rigid_regions_index = 0, rigid_counter = 0, a = 0;

  int *rigid_regions = NULL;

  clusters *nc_current = NULL;

  static int first_res_no, static_counter;

  /* print the Hydrogen - bond information on the left */
  // printf("\n----%d th run ------\n",++static_counter);
  fprintf(text_file, "%4d %7.3f %7.3f\t[  ", hb_number, hb_energy,
          mean_coordination);
  ...
  for (rigid_regions_index = 0; rigid_regions_index < number_of_residues * 3;
       rigid_regions_index++) {
  ...
  for (a = 0; a < rigid_counter; a++) {
    ...
    if (rigid_regions[a] < rigid_regions[a + 1] - 1)
      ...
      fprintf(text_file, " (%d--%d) ", rigid_regions[a] + 1,
              rigid_regions[a + 1] - 1);
  }
