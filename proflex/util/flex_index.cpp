...
//-------------------------------------------------- preset names for now
   ...
   */
  string RCcolors[] = {"","red","cyan","green","yellow","purple","purpleblue",\
			"brightorange","tv_red","tv_green","olive",\
			"deeppurple","blue","tv_blue","grey70","limon",\
			"hotpink","violet","ruby","yelloworange","forest"};

  string FCcolors[] = {"","wheat","palegreen","lightblue","paleyellow",\
			"lightpink","palecyan","lightorange","bluewhite",\
			"teal","limegreen","pink","slate","aquamarine",\
			"lightorange","sand","tv_orange","lime","marine",\
			"splitpea","salmon"};
...
//  if (argc >= 2 && strcmp(argv[1], "-1") == 0) {
      //Don't scale the output to 0 to 99 if "-1" is given as the command line argument
//      noscaling = 1;
//  }
...
//------------------------------------------------- find min flexibility label
	 ...
	 (label[atm2[i]-1] == count))
	{
      ...
      if(clst[i] < min_label) min_label = clst[i];
      if(clst[i] > max_label) max_label = clst[i];
    }
...
/* 
 * clst[] contains the cluster label for the various bonds in the allbonds file
 * So, we first map the bond labels onto the atoms such that a -ve label implies
 * that an atom belongs to a rigid cluster; +ve implies flexible cluster; and
 * ZERO for uncorrelated motion. This info is in label[].
 *
    ...
    count = 0;
     while( !input2.eof() && !input2.fail())
     {
	  ...
	  if(flagHP && !strncmp(line+13,"X ",2) && !strncmp(line+17,"XXX",3))
	   ...
	   {
	     for(i=0; i<6; i++) output1 << *(line+i);
	     output1 << setw(5) << setiosflags(ios::fixed | ios::right) 
	            << atmIndex[count];
	     for(i=11; i<54; i++) output1 << *(line+i);
...
//  output.open(file_flex_pml,ios::out);
  ...
  output<<"cmd.color( 's170', 'FlexIndexObj and b= 40.0')"<<endl;
  output<<"cmd.color( 's200', 'FlexIndexObj and b> 40.0 and b< 42.0')"<<endl;
  output<<"cmd.color( 's200', 'FlexIndexObj and b= 42.0')"<<endl;
  output<<"cmd.color('s225', 'FlexIndexObj and b> 42.0 and b < 44.0')"<<endl;
  output<<"cmd.color('s225', 'FlexIndexObj and b= 44.0')"<<endl;
  output<<"cmd.color('s250', 'FlexIndexObj and b> 44.0 and b < 46.0')"<<endl;
  output<<"cmd.color('s250', 'FlexIndexObj and b= 46.0')"<<endl;
  output<<"cmd.color('s280', 'FlexIndexObj and b> 46.0 and b < 48.0')"<<endl;
  output<<"cmd.color('s280', 'FlexIndexObj and b= 48.0')"<<endl;
  output<<"cmd.color('s310', 'FlexIndexObj and b> 48.0 and b < 49.0')"<<endl;
  output<<"cmd.color('s310', 'FlexIndexObj and b= 49.0')"<<endl;
 
  output<<"cmd.color('grey', 'FlexIndexObj and b> 49.0 and b < 50.0')"<<endl;
  output<<"cmd.color('grey', 'FlexIndexObj and b= 50.0')"<<endl;
 
  output<<"cmd.color('s690', 'FlexIndexObj and b> 50.0 and b < 52.0')"<<endl;
  output<<"cmd.color('s690', 'FlexIndexObj and b= 52.0')"<<endl;
  output<<"cmd.color('s730', 'FlexIndexObj and b> 52.0 and b < 54.0')"<<endl;
  output<<"cmd.color('s730', 'FlexIndexObj and b= 54.0')"<<endl;
  output<<"cmd.color('s760', 'FlexIndexObj and b> 54.0 and b < 56.0')"<<endl;
  output<<"cmd.color('s760', 'FlexIndexObj and b= 56.0')"<<endl;
  output<<"cmd.color('s790', 'FlexIndexObj and b> 56.0 and b < 58.0')"<<endl;
  output<<"cmd.color('s790', 'FlexIndexObj and b= 58.0')"<<endl;
  output<<"cmd.color('s820', 'FlexIndexObj and b> 58 and b < 60.0')"<<endl<<endl<<endl;
  output<<"cmd.color('s820', 'FlexIndexObj and b= 60.0')"<<endl;
  output<<"cmd.color('s850', 'FlexIndexObj and b> 60.0')"<<endl<<endl<<endl;
...
//  output2.open(file_RC_pml,ios::out);
  ...
  for ( i = (min_label*-1)-1, j = 1; i >= 0; i--, j++ )
  {
     ...
     {
	output2<<"\nselect RC"<<j<<", RigidClustObj and id "<<clstBoundary[i-1]+1<<"-"<<clstBoundary[i];        
	output2<<"\ncolor "<<RCcolors[(j%20)+1]<<", RC"<<j;

//	output2<<"\npreset.ball_and_stick('RC"<<i<<"')"<<endl;
// 	Just show the main-chain by default as a ribbon or "cartoon" in PyMol
  ...
  for ( i = (min_label*-1)-1, j = 1; i >= 0; i--, j++ )
  {
        ...
        if(flag == 'F' )
	{
	  ...
	  output2<<"\nselect smallRCs"<<j/50<<", RigidClustObj and id "<<clstBoundary[i-1]+1<<"-"<<clstBoundary[i];
	  flag = 'T';
	}
	else
	{
...
//	one cluster that is colored white
  output2<<"\n\nselect FC, RigidClustObj and id "<<clstBoundary[(min_label*-1)-1]+1<<"-"<<clstBoundary[(min_label*-1)+max_label];
  output2<<"\ncolor white, FC";
  output2<<"\nshow lines, FC";
...
//  output3.open(file_FC_pml,ios::out);
  ...
  output3<<"load "<<file_flexibility<<", FlexClustObj"<<endl<<endl;
                                        // Load molecule (*flex*.pdb)
  // Process the flexible clusters => (min_label*-1) through (max_label-min_label)
  // Note that min_label is negative (at least -1). So, we have to
  // first start with the cluster that has label = -1 to label = min_label
  ...
  for ( i = (min_label*-1)+1, j = 1; i <= max_label - min_label ; i++, j++ )
  {
     if ( clstBoundary[i] - clstBoundary[i-1] > 6 )
     {
        output3<<"\nselect FC"<<j<<", FlexClustObj and id "<<clstBoundary[i-1]+1<<"-"<<clstBoundary[i];
...
//      Just show the main-chain by default as a ribbon or "cartoon" in PyMol
  ...
  for ( i = (min_label*-1)+1, j = 1; i <= max_label - min_label ; i++, j++ )
  {
	...
	{
          // The rest of the atoms will be colored with a default color
	  output3<<"\nselect smallFCs"<<j/50<<", FlexClustObj and id "<<clstBoundary[i-1]+1<<"-"<<clstBoundary[i];
          flag = 'T';
        }
        else
        {
...
//      All the RCs are grouped under one cluster and colored blue
  output3<<"\n\nselect RC, FlexClustObj and id 1-"<<clstBoundary[(min_label*-1)-1];
  output3<<"\ncolor blue, RC";
  output3<<"\nshow lines, RC";
...
//	All uncorrelated motions are grouped as one object and colored white
  output3<<"\n\nselect dangle, FlexClustObj and id "<<clstBoundary[(min_label*-1)-1]+1<<"-"<<clstBoundary[(min_label*-1)];
  output3<<"\ncolor white, dangle";
  output3<<"\nshow lines, dangle";
